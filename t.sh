#!/usr/bin/env bash
# N4 — Cloud Run (US only) VLESS gRPC → send URL to Telegram (5h END label)
set -euo pipefail
...
MAX_INSTANCES="${MAX_INSTANCES:-8}"       # ← keep ≤8 to stay within 16 vCPU quota
PORT="${PORT:-8080}"
...
GRPC_SERVICE_NAME="${GRPC_SERVICE_NAME:-n4-grpc}"
